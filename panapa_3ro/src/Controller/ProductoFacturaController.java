...
public class ProductoFacturaController {
    ...
    public void Create(FacturaProducto Factura) {
        ...
        Lista_Factura.get(Lista_Factura.size() - 1).setProductos(Factura.getProductos());
        userco.salvar_datos();
    }
    ...
    public ArrayList<String[]> ReadSelect(int Index) {
        ArrayList<String[]> Get = new ArrayList<>();
        for (int i = 0; i < getLista_Factura().get(Index).getProductos().size(); i++) {
            Get.add(new String[]{getLista_Factura().get(Index).getProductos().get(i).getNombre(),
                getLista_Factura().get(Index).getProductos().get(i).getCantidad() + "",
                (getLista_Factura().get(Index).getProductos().get(i).getPrecio() * getLista_Factura().get(Index).getProductos ().get(i).getCantidad()) + ""});
        }
        return Get;
    }
    ...
    private ArrayList<String[]> ListaInsumos(Calendar Dia) {
        ...
        for (FacturaProducto Fact : getLista_Factura()) {
            if (Fact.getFecha().get(Calendar.YEAR) == Dia.get(Calendar.YEAR) && Fact.getFecha().get(Calendar.MONTH) == Dia.get(Calendar.MONTH)
                ...
                for (Producto Insu : Fact.getProductos()) {
                    if (Array.size() > 0) {
                        boolean Cond = true;
                        for (String[] Ar : Array) {
